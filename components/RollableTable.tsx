...
function RollableTable(props: RollableTableProps) {
    ...
    }, [isRolling, selected, props.selected]);

    useInterval(
        () => {
            setRolling((rolling + 1) % props.rows.length)
        },
        isRolling ? 25 : null,
    )
    ...
    function rowbg(i: number): string {
        ...
        return i % 2 == 0 ? "bg-stone-100" : "bg-stone-300"
    }

    return <table style={{minWidth: 300}}>
        <thead className={ebGaramond.className}>
        <tr className="text-slate-50 bg-black">
            <th colSpan={2} className="p-2 uppercase font-bold">
                <div className="flex items-center">
                    <div className="basis-full">{props.title}</div>
                    <button className="opacity-90 hover:opacity-100" onClick={onRoll}><DiceIcon/></button>
                </div>
            </th>
        </tr>
        </thead>
        ...
        {props.rows.map((row, i) => {
            return <tr key={i} className={classNames(rowbg(i), "cursor-pointer", "hover:opacity-70")}
                       onClick={onClick(i)}>
                <th colSpan={1} className="px-2 py-1">{i + 1}</th>
                <td colSpan={1} className="px-2 py-1">{row}</td>
            </tr>
        })}
        </tbody>
    </table>
...
function DiceIcon() {
    return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="2em">
